...
/* Test: millis increases over time */
TEST(millis_increases) {
    ...
    int64_t ms2 = millis();
    ASSERT_MSG(ms2 >= ms1, "Second timestamp should be >= first timestamp");
    return 0;
}

/* Test: millis has millisecond precision */
TEST(millis_precision) {
    int64_t ms1 = millis();
    ...
    /* Should have advanced by at least 1ms, but not more than 1 second */
    ASSERT_MSG(diff >= 0 && diff < 1000, "Time difference should be in millisecond range");
    return 0;
}
